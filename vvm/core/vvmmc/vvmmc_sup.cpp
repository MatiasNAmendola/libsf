...
//////
// Change log:
//     November 23, 2012 - Initial creation
//////
...
//////
	SVariable* ivvmmc_searchVariablesByName(SStartEnd* tseVariables, SDatum* tsDatum)
	{
		...
		if (tseVariables)
		{
			...
			return((SVariable*)oss_SEChain_searchByCallback(tseVariables, &cb));
...
//////
	bool iivvmmc_validateStartEndCompsCallback(SStartEndCallback* cb)
	{
		...
		if (cb && cb->ptr)
		{
			...
			oss_SEChain_validate(&line->comps, 0);
			oss_SEChain_validate(&line->compsCombined, 0);
...
oss_SEChain_validate(&lsf->lines, &cb);
